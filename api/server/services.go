...
func (c *clusterApi) serviceEnterMaintenanceMode(w http.ResponseWriter, r *http.Request) {
	...
	err = inst.ServiceEnterMaintenanceMode(false)
	if err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusInternalServerError)
		return
	}

	// should here msg requires rather than simple status ok reply?
...
func (c *clusterApi) serviceExitMaintenanceMode(w http.ResponseWriter, r *http.Request) {
	...
	err = inst.ServiceExitMaintenanceMode()
	if err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (c *clusterApi) serviceAddDrive(w http.ResponseWriter, r *http.Request) {
	...
	status, err := inst.ServiceAddDrive(srvReq.Operation, srvReq.Drive, srvReq.Journal)
	if err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (c *clusterApi) serviceReplaceDrive(w http.ResponseWriter, r *http.Request) {
	...
	status, err := inst.ServiceReplaceDrive(srvReq.Operation, srvReq.Source, srvReq.Target)
	if err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (c *clusterApi) serviceRebalancePool(w http.ResponseWriter, r *http.Request) {
	...
	status, err := inst.ServiceRebalancePool(srvReq.Operation, srvReq.PoolID)
	if err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusInternalServerError)
		return
	}
